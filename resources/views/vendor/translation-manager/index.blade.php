...
<head>
    ...
    <meta name="description" content="{{ trans('general.meta.description') }}">
    <meta name="keywords" content="{{ trans('general.meta.keywords') }}">
    <meta name="author" content="{{ trans('general.meta.author') }}">
    <link rel="icon" href="{{ secure_asset('favicon.ico') }}" type="image/icon">

    <title>Mineathon</title>
    <link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/css?family=Ruda:400,700,900">
    ...
    <section class="wrapper">
        <div class="col-md-12">
            ...
            <div class="alert alert-success success-publish" style="display:none;">
                <p>{{ trans('transSystem.published', ['group' => $group]) }}</p>
            </div>
        ...
        <div class="col-md-6">
            ...
            @if($group)
                <br><h4>Total: {{ $numTranslations }}, changed: {{ $numChanged }}</h4>
            @endif
        </div>
        @if(!isset($group))
            <div class="col-md-6">
                <form class="form-find pull-right" method="POST"
                    ...
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="submit" style="margin-left:5px" class="btn btn-info"
                            data-disable-with="Searching..">{{ trans('transSystem.findFiles') }}</button>
                </form>
                <form class="form-import pull-right form-inline" method="POST"
        ...
        @if($group)
            <div class="col-md-6">
                <form class="form-publish pull-right" method="POST"
                    ...
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <button type="submit" style="margin-left:5px" class="btn btn-info"
                            data-disable-with="{{ trans('transSystem.publishing') }}..">{{ trans('transSystem.publish') }}</button>
                </form>
                <form action="{{ action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) }}"
                      method="POST" role="form" class="form-inline pull-right">
                    <div class="form-group">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <textarea class="form-control" rows="1" name="keys"></textarea>
